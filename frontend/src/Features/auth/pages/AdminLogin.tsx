...
export default function AdminLogin({ onBackClick, onAdminLoginSuccess }: AdminLoginProps) {
  // Input fields state tracking matching your system schemas
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Fire the success handler to authorize the organizer console workspace
    onAdminLoginSuccess();
  };
    ...
    <div className="min-h-screen w-full bg-[#f8fafc] bg-gradient-to-b from-white to-[#f1f5f9] flex flex-col justify-between font-sans antialiased selection:bg-slate-200">
      ...
      <main className="flex-grow flex items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-md border border-slate-200/60 p-6 sm:p-8 space-y-6 animate-fade-in">
          ...
          {/* Header Branding Panel (Matches image_55cac9.png layout) */}
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4">
            ...
            {/* Password Field with Inline Visibility Toggle and Recovery Shortcut */}
            <div className="space-y-1.5">
              <div className="flex justify-between items-center select-none">
                ...
                <span 
                  onClick={() => alert("Contacting security operations room recovery desks...")}
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-[#f8fafc] border border-slate-200 hover:border-slate-300 rounded-lg p-2.5 pr-10 text-xs font-semibold text-slate-700 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-950/5 focus:bg-white focus:border-slate-950 transition-all"
                />
            ...
            <div className="flex items-center gap-2 pt-1 select-none">
              <input 
                type="checkbox" 
                id="remember"
                className="w-3.5 h-3.5 border-slate-300 rounded focus:ring-0 cursor-pointer"
              />
              <label htmlFor="remember" className="text-[11px] font-semibold text-slate-400 cursor-pointer">
            ...
            <div className="pt-2">
              <button
                type="submit"
                className="w-full bg-[#030a1c] hover:bg-slate-900 text-white font-extrabold text-xs py-3 px-4 rounded-xl shadow-sm transition-all duration-150 cursor-pointer flex items-center justify-center gap-1 active:scale-[0.98]"
              >
